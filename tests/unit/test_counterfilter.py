import unittest
import logging


from src.filters.filter_counter import CountFilter
from src.sketchgraphs.sketchgraphs.data.sequence import EdgeOp, NodeOp
from src.filteringpipeline.src.filters import KO_FILTER_TAG

logging.basicConfig(level=logging.DEBUG)
logger = logging.getLogger()


class TestCountFilter(unittest.TestCase):

    def test_counter(self):
        # Test max nodes, should not send KO
        n_nodes = 4
        self.mock_sequence_1 = [NodeOp(label=0)]*n_nodes + [EdgeOp(label=0, references=None)]*2
        conf_dict_1 = {'min' : None, 'max' : 5, 'type': 'node'}
        filter1 = CountFilter(conf_filter=conf_dict_1)
        for op in self.mock_sequence_1:
            message = {'op': op}
            filter1.process(message)
        self.assertEqual(filter1.count, n_nodes)
        answer = filter1.last_process(message)

        self.assertIsNone(answer.get(KO_FILTER_TAG))
        
                
        # Test min edges, should send KO
        self.mock_sequence_2 = [NodeOp(label=0)]*5
        conf_dict_2 = {'min' : 0, 'max' : None, 'type' : 'edge'}
        filter2 = CountFilter(conf_filter=conf_dict_2)
        for op in self.mock_sequence_2:
            message = {'op': op}
            filter2.process(message)
        self.assertEqual(filter2.count, 0)
        answer = filter2.last_process(message)
        tag = answer.get(KO_FILTER_TAG)
        
        self.assertEqual(tag, filter2.name)